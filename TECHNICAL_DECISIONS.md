...
| **The offline proof runs `--pull never`, and the bundle overlay deletes the build recipe rather than suppressing it** | The diagnosis still holds: `compose.tools.yml` carries a `build:` for the proof runner, so a missing `aow-bundle/demos:<commit>` tag makes Compose *build* instead of failing — and the build's `apk add` then needs exactly the egress the proof exists to show is absent, so a tag problem would have been reported as a network error. The first remedy did not hold: `docker compose run` has no `--no-build`, and Compose v5.5.1 rejects it with `unknown flag: --no-build`, so on a v5 target the wrapper aborted before the proof started (`up --no-build` is supported, and is still used where a stack is started). So `compose.tools.bundle.yml` sets `demos.build: !reset null`, which *removes* the recipe instead of asking Compose to suppress it, and `scripts/package-offline.sh` refuses to seal a bundle whose rendered `demos` service still has one. A proof has to be able to fail for the reason it is testing |
