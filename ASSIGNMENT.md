...
- **Not every score is worded.** Scoring every activity is free; asking a CPU-bound local model to write a sentence about each of ~1,300 rows is hours. The consumer ranks each city-day and queues only the top `ENRICH_TOP_N` (default 6) for wording; the rest are stored `deferred` — scored, charted and answerable, just not written about. Asking about one by name, or the re-word control in the UI, promotes it. This is a cost decision, and the UI states it rather than showing a blank. The model server runs with `--parallel 2` for the same reason: with a single slot an interactive question queued behind an enrichment batch and timed out as though the model were down.

**Content and language**

- **Cities.** The shipped selection is Rome, London, Lisbon, Tel Aviv and Reykjavík. Rome and London make both illustrative questions work as written; the coastal cities support sea-adjacent activity examples, and Reykjavík adds a contrasting daylight/weather case. The stored coverage is shown per city in the UI and `/coverage`; it is uneven and changes with refreshes.
...
- **Nothing is invented, and samples say so.** No event, concert or fixture is passed off as real. Every record carries a source and an as-of date, and a question outside the data coverage gets an explicit "no data for that date" answer. Real listings live in `data/events.seed.jsonl`, hand-verified against their own source URLs — the venue's or the organiser's own listing page, never a venue page treated as proof that something is scheduled there. There are 55, covering all five cities but unevenly (london 10, rome 10, tel-aviv 8, reykjavik 13, lisbon 14), and the documentation states that spread rather than the total.
- **Generated sample events are a demo mode, not the default.** Hand-checking covers the days around the snapshot, not a whole season, so we also generate `data/events.samples.jsonl`: 45 rows, every one `is_sample: true`, titled "Sample: …", anchored to a real venue from the places snapshot, and declared as not-a-real-listing in its `source` field. Their URL is the venue's own record and is rendered as a **venue reference**, never as the event's source, because there is no listing to point at. Our choice is that they are **opt-in**: `docker compose up -d` stores the verified rows and nothing else, and `compose.demo.yml` (`make up-demo`) turns the samples on behind a banner in the UI. The ingestor never accepts them without the flag; the consumer — the only role with write grants — drops one that arrives without it and deletes every `is_sample` row on startup when the flag is off, so a database cannot keep demo rows after leaving demo mode. Counts are reported as "55 verified + 45 samples", never as one total. Reason: labelling is enough for honesty, but a reviewer reading a fabricated-looking row in the default database has to trust the label; a default run that simply does not contain one needs no trust.
- **A stored event expires, and says so.** The brief asks that nothing be invented; it says nothing about how long a true statement stays true. Our choice: an event row is a reading of a listing page taken on a particular day, so it records `checked_at` and carries a `valid_until` derived from it (`checked_at + AOW_EVENT_RECHECK_DAYS`, default 21 days). Past that it is still stored, still counted and still shows its source, but it is no longer returned as a currently scheduled event, and a gap caused by that filter says so rather than implying the city is quiet. Reason: an air-gapped run cannot discover that a venue cancelled a show, so the only honest thing it can do is state how old its reading is and stop short of presenting it as a schedule. Cost: a snapshot left alone long enough reports zero current events — the same bargain the 16-day weather window already makes, and closed the same way, by a connected re-check.
- **A sea activity's score is capped at what the data can support.** The brief names surfing as an example activity, and our rules score it from wind, temperature and rain. Nothing in this system measures waves, swell or water temperature, so surfing, swimming, fishing and a boat ride are capped at 69 — one point below the `good` band floor — and can never be reported as a confident recommendation; a day at the beach is exempt, because shore weather is exactly what is measured. Each coastal city also names a real point on its coast, and the stored distance from the forecast point to it (Tel Aviv 1.4 km, Reykjavík 2.6 km, Lisbon 17.8 km, Rome 24.7 km) is printed wherever one of those scores appears. Reason: `coastal: true` was an assertion nobody could check, and Rome's rested on a beach 25 km from the forecast point. The rule is stronger than the cap: no scoring rule may infer a sea state from a land measurement at all, which is why surfing lost the `min_wind_kmh` that used to call a calm day "too flat for this" (`rule_version` 4). Rejected alternative: staging a marine provider. Open-Meteo publishes a keyless marine endpoint and it was probed rather than assumed — it answers for all four coast points with real wave and swell values, but only for 10 of the 16 days this system stores, and from its own grid cell 2.9–13.2 km away. It would **narrow** the gap, not close it, so six days in sixteen would stay capped anyway; against that it puts a second source into the offline bundle and the connected refresh path.
...
- Pinned image versions (digests where practical), containers run as non-root where the image allows, no published ports beyond the UI, API and Grafana, and an image scan in CI. **As built:** digests everywhere, uid 10001 where the base image allows, and an image scan in CI. The default stack publishes the UI and API on `127.0.0.1`; the optional monitoring overlay also publishes Grafana on `127.0.0.1:3000` and provisions three dashboards.
