...
- **Nothing is invented, and samples say so.** No event, concert or fixture is passed off as real. Every record carries a source and an as-of date, and a question outside the data coverage gets an explicit "no data for that date" answer. Real listings live in `data/events.seed.jsonl`, hand-verified against their own source URLs; there are seven and they are all in London.
- **Generated sample events are a demo mode, not the default.** A planner that can only be exercised in one city out of five demonstrates nothing, so we also generate `data/events.samples.jsonl`: 45 rows, every one `is_sample: true`, titled "Sample: …", anchored to a real venue from the places snapshot, and declared as not-a-real-listing in its `source` field. Their URL is the venue's own record and is rendered as a **venue reference**, never as the event's source, because there is no listing to point at. Our choice is that they are **opt-in**: `docker compose up -d` stores the seven verified rows and nothing else, and `compose.demo.yml` (`make up-demo`) turns the samples on behind a banner in the UI. The ingestor never accepts them without the flag; the consumer — the only role with write grants — drops one that arrives without it and deletes every `is_sample` row on startup when the flag is off, so a database cannot keep demo rows after leaving demo mode. Counts are reported as "7 verified + 45 samples", never as one total. Reason: labelling is enough for honesty, but a reviewer reading a fabricated-looking row in the default database has to trust the label; a default run that simply does not contain one needs no trust.
