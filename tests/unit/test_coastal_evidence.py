...
# Inland, and therefore with no coast block and no coast columns at all. It is
# here so the contrast can be asserted: a city with no coastal row gets the
# stronger statement instead of the caveat, and the four `coast_*` keys are
# absent rather than null-ish, which is how `queries.cities` returns them.
LONDON = {
    "id": "london",
    "name": "London",
...
def ask(monkeypatch, activities):
    ...
    monkeypatch.setattr(router.queries, "cities", lambda _conn: [LONDON, ROME, TEL_AVIV])
    monkeypatch.setattr(router.queries, "coverage", lambda _conn: COVERAGE)
    monkeypatch.setattr(router.queries, "events", lambda *_a, **_k: [])
    # Asked only when the retrieval above comes back empty, so a gap can say
    # whether the event feed went stale or was never there (F9's other half).
    monkeypatch.setattr(
    ...
    def recommendations(_conn, city_id=None, *, start=None, end=None, activity=None):
        if activity not in activities:
            return []
        # The coastal gate, as the pipeline applies it: `rules.activities_for_city`
        # drops a `requires_coast` activity for an inland city, so no row is ever
        # created and the reader finds nothing rather than a number.
        inland = {city["id"] for city in (LONDON, ROME, TEL_AVIV) if not city["coastal"]}
        if activities[activity].get("requires_coast") and city_id in inland:
            return []
    ...
    def answer(question: str, *, expect_scores: bool = True) -> str:
        """`expect_scores` guards against a test passing because the retrieval
        quietly returned nothing. It is turned off for the one case where an
        empty retrieval is the behaviour under test: an inland city has no
        coastal row to find, and the answer is built from that absence."""
        ...
        if expect_scores:
            ...
            assert not result.recommendations, f"expected no scores for {question!r}"
            assert result.unscored_activities, f"nothing reported as unscored for {question!r}"
...
def test_an_inland_city_gets_the_stronger_statement_and_not_the_caveat(ask):
    """London has no coastal row at all, so there is no score to qualify.

    "London has no coast on record" is a stronger and simpler thing to say
    than a note about what a score does not measure, and following it with one
    would be qualifying scores that do not exist -- which reads as though some
    number had been computed after all."""
    answer = ask("is it good for surfing in london this week", expect_scores=False)
    assert "no coast on record" in answer
    assert "nothing in the data measures" not in answer
    assert "forecast point" not in answer
