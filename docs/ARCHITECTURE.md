...
| **Refresh when connected** | The operator refresh does not use the declared `egress` network. `scripts/refresh.sh` creates an ephemeral `<project>_refresh_egress`, attaches only the ingestor to it for the length of one fetch, then detaches it and deletes the network, asserting both. Newly accepted weather follows the normal outbox and queue path. | [`scripts/refresh.sh`](../scripts/refresh.sh), [connected refresh](../README.md#connected-refresh); the declared `egress` network belongs to [`compose.connected.yml`](../compose.connected.yml) |
